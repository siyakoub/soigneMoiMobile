...
class PrescriptionService {
  ...
  Future<Map<String, dynamic>> getAllPrescriptionByMedecin(String medecinId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/prescriptions/$medecinId/bymedecin'),
      headers: {
        'Content-Type': 'application/json'
      }
    );
  ...
  Future<Map<String, dynamic>> createPrescription(PrescriptionCreation prescriptionCreation) async {
    final response = await http.post(
      Uri.parse('$baseUrl/prescriptions/new'),
    ...
    if (response.statusCode == 201) {
      ...
      throw Exception('Prescription invalide...' + response.body);
    } else {
